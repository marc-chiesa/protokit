...
@skip_under_pure_python(
    ...
    "recursively, one frame per import level, so registering the 1200-deep chain "
    "raises RecursionError against the interpreter's default limit of 1000 before "
    "protokit's iterative walker ever runs; a runtime limit, not a protokit defect, "
    "so a permanent backend skip rather than an inventory entry (U2, KTD10)"
)
