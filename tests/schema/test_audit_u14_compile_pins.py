...
* :func:`~protokit.schema.compile._detect_root_transitive_shadow`
  (``compile.py:319``, loop head ``for root in paths:`` at ``:339``)
...
**U14-1** (``compile.py:356``, ``candidate = Path(inc) / root.name``). The
...
**U14-2** (``compile.py:370-400``). ``_detect_same_basename_collision``
...
class TestU141BasenameShadowDetection:
    ...
    @pytest.mark.xfail(
        ...
        reason=(
            "U14-1: _detect_root_transitive_shadow (compile.py:356) probes "
...
class TestU142SameBasenameCollisionIgnoresProtoPaths:
    ...
    @pytest.mark.xfail(
        ...
        raises=AssertionError,
        reason=(
            "U14-2: _detect_same_basename_collision (compile.py:370) takes "
