...
**Empirical foundation:** 14 buf v1.69.0 NDJSON snapshots committed at `tests/schema/lint/rules/fixtures/package_same/_buf_smoke/recorded/` — 7 initial + 7 supplementary (added 2026-05-17 to verify cross-rule homogeneity, sort order, and bool rendering per document-review's 3-persona convergence). Every architectural decision below cites the corresponding recorded snapshot.
...
All architectural decisions below cite specific recorded NDJSON snapshots from `tests/schema/lint/rules/fixtures/package_same/_buf_smoke/recorded/` (committed as foundational research artifacts, not part of the regular test run). The 14 fixtures are:
...
| `reverse-order-go` | 3 files, `go_package = "github.com/x/Y/X/Y"` (a=Y, b=X, c=Y) | Fires on all 3 with `"multiple values "github.com/x/X,github.com/x/Y""` | **DECISIVE: buf sorts alphabetic-by-value, NOT filename-order or first-encountered.** Protokit's `sorted(declared_values)` decision is empirically locked. If buf used filename-order or first-encountered, output would have been `"github.com/x/Y,github.com/x/X"` (a's Y first). |
...
- **Cross-rule mixed-presence template** — supplementary smoke covered mixed-value only. If U6 reveals divergence on the mixed-presence template for any non-go_package rule, document via `_PARITY_EXCEPTIONS`. Low risk given mixed-value uniformity.
