...
- **PD-6 — REVISED per U3 Phase 0 (2026-05-22): U3 emission shape is per-import-edge**, not per-root-file fan-out. Phase 0 empirically bound buf v1.69.0's behavior on package-level cycles: one finding per file containing a cycle-closing `import` statement, with the offending import's line/column as the finding location, and message `"Package import cycle: <cycle path starting at self's package>"`. The original PD-6 (per-root-file fan-out) would have over-emitted on sibling "leaf" files in cyclic packages that don't import anything in the cycle. The revised binding matches buf's finding SET exactly + addresses the over-emission UX concern raised at ce:review time (KD-1: protokit-UX overrides buf-parity when they conflict; here both point the same way).
- **PD-7 — REVISED per U3 Phase 0 (2026-05-22): U3 cycle scope is per-import-edge within an SCC of size ≥ 2.** For each cycle-closing import edge (i.e., an import whose source package and target package are both in the same SCC), emit one finding on the importing file pointing at the import statement's line/column. The original PD-7 ("fire if ANY root file participates") was at the wrong granularity — buf's actual scope is more precise. Files that share a cyclic package but don't have any cycle-closing imports themselves do NOT emit findings.
- **PD-8 — REVISED per U3 Phase 0 (2026-05-22): `FileLintContext.import_cycles` shape is `Mapping[str, tuple[CycleEdge, ...]] | None = None`**, where `CycleEdge` is a frozen dataclass with `(imported_file: str, target_package: str, cycle_path: tuple[str, ...], line: int | None, column: int | None)`. The mapping is keyed on root file name; the value is the tuple of cycle-closing imports in that file. Line/column derive from `SourceCodeInfo.Location` keyed on `path=[3, dependency_index]` (3 = `FileDescriptorProto.dependency` field number); if `include_source_info=False` at compile time (e.g., descriptor-set input without source info), line/column default to None and the rule emits at FileLocation without precise location info.
- **PD-12b (NEW from U3 Phase 0)**: Extend `FileLocation` with optional `line: int | None = None` + `column: int | None = None` fields. Open-extension change (existing FileLocation consumers ignore the new fields; new consumers use them when present). Does NOT bump `_LINT_JSON_SCHEMA_VERSION` per [[closed-literal-discriminator-bump-trigger-2026-05-17]] (not a closed-Literal discriminator addition). JSON / SARIF / JUnit formatters render `location_line` / `location_column` keys when present, null when None. Required for U3 to emit at byte-equivalent buf parity (Option B from ce:review session 2026-05-22 user decision).
- **PD-14 (NEW from U3 Phase 0)**: Two-tier behavior in buf v1.69.0 — file-level cycles (file A imports file B; file B imports file A) are caught at the COMPILE phase as `type:"COMPILE"` errors (protoxy AND protoc both reject); only **package-level cycles with file-level acyclic imports** reach the `PACKAGE_NO_IMPORT_CYCLE` lint rule. The original brainstorm assumed file-level cycles were the rule's primary target; Phase 0 reveals the rule's actual ground is the more subtle case (e.g., `pkg_a/a1.proto` imports `pkg_b/b1.proto` and `pkg_b/b2.proto` imports `pkg_a/a2.proto` — file-level acyclic, package-level cyclic). U3 fixtures must focus on this case; file-level cycle fixtures are dropped (unreachable for the lint rule).
...
- **PD-12 — No `_LINT_JSON_SCHEMA_VERSION` bump in D6e** (binds KD-9): no new closed-Literal `LintRuntimeWarning.category` values added; no new `LintLocation` discriminant variants (U3 extends existing `FileLocation` with optional fields per PD-12b — open extension, not a closed-Literal addition). The bump-contract ratchet substrings at `tests/test_builtin_lint_formatter.py:705-760` do not need editing unless prose around the constant changes.
