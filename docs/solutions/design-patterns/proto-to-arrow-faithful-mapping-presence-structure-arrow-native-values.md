...
- [Detecting unmodeled wire data via a DiscardUnknownFields byte-delta probe](unmodeled-wire-data-probe-byte-delta-blind-to-declared-extensions.md) — the *third, out-of-band* fidelity channel referenced in Context: it detects wire data the descriptor does not model (which neither axis here covers), and names its own blind spot: a *declared* proto2 extension reads into `Extensions[...]` with an empty unknown set, so the probe misses it (v2 adds a structural oracle to close this). A group field is *not* this case — ptars emits a column for it; a populated group is a separate decode fault.
- Origin: PR #17 (`feat/storage-pr3-columnar`); design recorded in `docs/brainstorms/2026-06-02-storage-pr3-columnar-parquet-requirements.md` (R5–R7, R11–R13) — maintainer-local and not in this repo.
