...
- [Structural fidelity oracle: descriptor-vs-produced-schema diff](structural-oracle-declared-extensions-descriptor-vs-encoder-schema-diff.md) — the v2 oracle that **closes the declared-extension blind spot named above**: a bind-time diff of the descriptor's declared extensions against ptars's produced columns. The deliberate complement to this per-record probe — undeclared drift (here) ↔ declared-but-uncolumnarized drift (there), each with a stated domain of validity.
