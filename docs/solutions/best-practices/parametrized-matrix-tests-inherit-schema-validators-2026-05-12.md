...
- audit-wire-format-before-claiming-sibling-parity-2026-05-03 (maintainer-side learning, kept in the private learnings repository rather than this tree) — the planning-time discipline of asserting wire-format parity; this learning is the test-time counterpart that closes the loop
...
- [[rule-pack-extension-ssot-rule-ids-and-test-class-naming-2026-05-12]] — parallel single-source-of-truth discipline for rule-pack extension. Deriving `_ALL_RULE_IDS` from `module.RULES` (rather than hand-listing strings) is the same shape as inheriting schema validators from a shared fixture (rather than re-declaring per cell): both eliminate an invisible update obligation when sibling tests extend a common surface.
