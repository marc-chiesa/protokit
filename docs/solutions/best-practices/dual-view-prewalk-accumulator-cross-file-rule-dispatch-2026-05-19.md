...
- `src/protokit/schema/lint/engine.py::_build_directory_package_accumulator` — reference implementation (commit `94eb76d`). Method docstring enumerates: (a) `root_files` scope rationale + "DO NOT unify" counterfactual, (b) frozenset-vs-MappingProxyType immutability contract, (c) co-reset discipline.
