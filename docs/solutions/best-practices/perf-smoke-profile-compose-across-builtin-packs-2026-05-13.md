...
**Before** — `tests/schema/lint/test_perf_smoke.py` lines 145-151 at commit `e555bd4` (the D6a U4 feature commit, before this fix):
...
**After** — same file lines 145-162 at commit `d6690e1` (the D6a U4 ce:review follow-up):
...
- audit-wire-format-before-claiming-sibling-parity-2026-05-03 (maintainer-side learning, kept in the private learnings repository rather than this tree) — adjacent: the audit discipline asks "is the claim still true?" Applied here as "is the test still measuring what it claims to measure after the canonical collection grew?"
- [[cli-fixture-proto-hygiene-must-satisfy-builtin-packs-2026-05-13]] — fixture-side counterpart of the same BUILTIN_PACKS-growth failure mode. This doc covers the *profile-composition* side (perf-smoke profile pinned to `BUILTIN_PACKS[0]` silently drops new pack rules); the sibling covers the *fixture* side (CLI fixtures constructed before new rules existed silently break under them). Both share the "BUILTIN_PACKS growth as invisible update obligation" core insight at different artifact layers.
