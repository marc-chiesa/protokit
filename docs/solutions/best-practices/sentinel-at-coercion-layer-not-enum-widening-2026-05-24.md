...
D6f U2 (commit `6fdeaf8`) implemented R9b per-rule disable for protokit-lint. One of the five disable mechanisms is `[tool.protokit.lint.severities] <rule_id> = "off"` — users who already have a `severities` table in their pyproject can suppress a rule by setting its severity to the sentinel string `"off"`.
...
`src/protokit/schema/lint/_config.py` (D6f U2, commit `6fdeaf8`):
