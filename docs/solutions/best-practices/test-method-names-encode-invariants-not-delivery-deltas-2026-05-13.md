...
**Before** — `tests/schema/lint/cli/test_cli_profile_resolution.py` at commit `e555bd4` (the D6a U4 feature commit, where the test was first inverted with the wrong name):
...
**After** — same file lines 276-306 at commit `d6690e1` (the D6a U4 ce:review follow-up):
...
The method name pins the invariant (`_two_packs` describes the gate-firing condition); the docstring captures the delivery history without polluting the test-ID surface. The sibling method `test_multi_pack_emits_provenance_line` followed the same timeless convention from the start and needed no rename — the U4 rename brought the inverted test into line with that existing convention. (Postscript: the real test was later renamed to `..._multi_pack` when the fixture grew past two packs — the invariant itself changed, which is the one legitimate reason an invariant-shaped name changes; the delivery-delta suffix still never appeared.)
...
- apply-institutional-learnings-postdating-plan-during-ce-review-2026-05-09 (maintainer-side learning, kept in the private learnings repository rather than this tree) — adjacent: the discipline of absorbing post-plan ce:review findings into the docstring (rather than the method name) is one application of the "docstring holds the history, method name holds the invariant" split this doc generalizes.
