...
- [`no-raise-contract-extends-to-post-init-failures-2026-05-14.md`](no-raise-contract-extends-to-post-init-failures-2026-05-14.md) — when this paired-field-invariant logic raises (e.g., `raise ValueError("payload set without source")`), and the dataclass is returned from a function with a documented no-raise contract, the dispatch tree must wrap the final `DataClass(...)` construction too — otherwise the invariant-violation exception escapes the contract silently. The fallback path passes safe sentinels so the rebuild cannot trip the same invariant.
...
- [[semantic-category-conflation-accepted-tradeoff-literal-widening-2026-05-13]] — `LintRuntimeWarning` is this learning's anchor frozen dataclass. That doc explains why one of its `Literal` fields (`category`) was reused at a second emit site (D6a U9 R9a CLI synthesis) rather than widened with a new value, when widening would have been a wire-format change. Same dataclass, adjacent design pressure: this doc covers field-relationship invariants via `__post_init__`; that doc covers schema-evolution decisions on the `Literal` fields the dataclass carries.
