...
| **Frozen planning artifact** | The hit lives in `docs/plans/`, `docs/brainstorms/`, or a learning under `docs/solutions/` that documents a past pattern. The forward-looking language was correct at authoring time and is deliberately preserved as a snapshot. `docs/plans/2026-05-04-...-d3-cli-plan.md` containing "until D6 ships". | **Leave as-is.** Plans and brainstorms are snapshots; refer to [[apply-institutional-learnings-postdating-plan-during-ce-review (maintainer-side learning, kept in the private learnings repository rather than this tree)]] for how plan-vs-implementation drift surfaces at ce:review (the plan's forward-looking language is the planning record, not a runtime-discoverable claim). |
| **Self-referential delivery target** (added 2026-05-19, D6c U5) | A `deferred to X` code comment lands in delivery X without being executed by X. The work was consciously left out of scope at X, but the comment now reads as if X picked it up. `engine.py:484`: `deferred to D6c if SC E7 benchmark exceeds 50ms` when D6c ships without exercising the lazy-gating threshold. `deprecated_replacement.py:58`: `deferred to D6c per the per-unit plan's K-9 decision` when D6c does not implement the absent-source-info LintCompileDiagnostic. | **Generalize, do NOT delete.** Two valid rewrites: (a) **measurable threshold** — replace `deferred to X` with `revisit only if <concrete condition>` (e.g., `revisit only if SC E7 benchmark exceeds 50ms`); (b) **delivery-agnostic phrase** — `deferred to a future delivery`. If the comment cites a specific decision document (`per the per-unit plan's K-9`), relocate to durable delivery+unit reference (`per the D6b U3 plan's K-9 decision`). Deleting the deferral entirely is wrong — the deferral itself is institutional memory that the work was considered and consciously left out. |
...
- dormant-code-changelog-draft-staging-delivery-boundary-2026-05-17 — Component 5 of that learning (`--help` discoverability line) enumerates the CLI epilog as one of the sweep sites this rubric must catch at U_final. D6b U7 swept the `"Opt into the dormant PACKAGE_SAME_* rule family (R7) — not in the default profile until 0.3.0"` epilog block as part of the boundary-unit deliverable; 12 sites total were swept in that commit.
- plan-review-verify-prior-art-citations-2026-05-15 — symbol-name drift (the U7 extension at the end of that learning) is a sibling staleness failure mode. This learning covers stale TEMPORAL phrasing (`"until 0.3.0"`, `"will land in U6"`); the companion learning covers BRAINSTORM-TO-IMPLEMENTATION symbol-name drift (`source_locations` documented but `source_info_descriptors` shipped; `_LintContextEmitMixin.leading_comment(path)` documented but `leading_comment(source_info_descriptors, file_name, path)` free function shipped). Both surface at delivery-boundary documentation passes; both need explicit verification steps to catch.
