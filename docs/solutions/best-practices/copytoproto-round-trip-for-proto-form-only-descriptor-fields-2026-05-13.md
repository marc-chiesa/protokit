...
extension (upb, the default) and the pure-Python runtime — though each backend chooses
...
|---|---|---|
| `FileDescriptorProto.public_dependency` | No on `FileDescriptor` | `imports/no-public` (D6a Unit 5, commit `5c8f2a4`) |
...
- audit-wire-format-before-claiming-sibling-parity-2026-05-03 (maintainer-side learning, kept in the private learnings repository rather than this tree) — buf-parity audit discipline. The D6a U5 imports rules' parity claims rest on documentation + behavioral reasoning, not empirical buf-binary cross-checks (parity test infrastructure is planned for D6a Unit 9). The next ce:review pass that touches buf-parity should validate the public/weak skip semantics against buf's actual IMPORT_USED output.
...
- [[no-raise-contract-extends-to-post-init-failures-2026-05-14]] — downstream consequence. The D6b R6a opt-in (`compile_protos_to_result(include_source_info=True)`) was the first surface forced to store full `FileDescriptorProto` instances on a `CompileResult` field — captured BEFORE `pool.Add()` per the constraint this doc establishes. That broadly-typed `Mapping[str, FileDescriptorProto] | None` field is precisely the surface that exposed the no-raise contract gap: any non-standard Mapping a caller passes can trip `__post_init__`'s `MappingProxyType(dict(...))` snapshot, escaping the dispatch contract unless the final `CompileResult(...)` construction is wrapped too.
- plan-review-verify-prior-art-citations-2026-05-15 — D6b U3 plan-review verified the capture-before-Add invariant documented here against the per-unit plan's K-8 paraphrase. The catch: the plan initially said "post-success insertion (AFTER pool.Add)" — which would have captured empty `source_code_info` on every fd because of the exact invariant this doc establishes. The plan-review-phase verify-don't-trust discipline is the workflow analog to the technical pattern documented here.
