...
- audit-wire-format-before-claiming-sibling-parity-2026-05-03 (maintainer-side learning, kept in the private learnings repository rather than this tree) — `module.RULES` is the canonical pack entry-point; this learning is the test-side reading of the same contract that document is the planning-side reading of.
...
- [[perf-smoke-profile-compose-across-builtin-packs-2026-05-13]] — parallel single-source-of-truth discipline applied at the test-input level. This doc says derive the rule_id frozenset from `RULES`; the sibling says derive the profile from iteration over `BUILTIN_PACKS` rather than pinning to `[0]`. Both eliminate an "invisible update obligation" when the canonical collection grows.
- [[test-method-names-encode-invariants-not-delivery-deltas-2026-05-13]] — natural sibling at the method-name granularity. This doc covers test *class* naming (deduplication across sibling files); the sibling covers test *method* naming (invariant-vs-delivery-moment encoding). Together they cover both granularities of test-naming hygiene.
- [[lint-rule-message-templates-must-not-recommend-actions-that-trigger-siblings-2026-05-13]] — extends the rule-pack-extension checklist with a cross-rule message audit: when adding a new rule, audit its `message_template` remediation advice against every sibling rule in the same shipping profile. The SSOT discipline this doc establishes ("derive from canonical source, not hand-list") generalizes naturally — the sibling rules ARE the canonical source for "what actions trigger findings"; deriving the message_template's suggested remediations from that source (by audit) follows the same principle as deriving rule_id sets from `RULES`.
- [[cli-fixture-proto-hygiene-must-satisfy-builtin-packs-2026-05-13]] — extends the rule-pack-extension checklist with a CLI fixture audit. When adding a new rule to a default-profile pack, grep `tests/schema/lint/cli/` for manually-constructed `FileDescriptorProto` fixtures and verify each one satisfies the new rule's constraint. The same "audit before merge" discipline that this doc applies to rule_id inventories applies to CLI fixture hygiene — both are invisible update obligations triggered by BUILTIN_PACKS growth.
