...
- plan-review-verify-prior-art-citations-2026-05-15 — D6b U3 plan-review caught a K-8 paraphrase that would have re-broken this no-raise contract's preconditions. The `source_info_descriptors` capture-after-Add ordering (the wrong version in the initial plan draft) would have populated the field with `FileDescriptorProto` objects whose `.source_code_info.location` arrays were empty — a soft-failure mode that doesn't trip the `__post_init__` wrap's raise path but breaks every downstream consumer. The plan-review-phase verify-don't-trust discipline complements this doc's run-time no-raise guard: catch invariant violations before they reach the `__post_init__` wrap, not after.
