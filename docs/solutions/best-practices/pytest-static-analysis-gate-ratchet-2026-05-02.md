...
- [[subprocess-exit-code-validation-test-harness-2026-05-13]] — this doc's `_run()` helper (which does `check=False` then explicitly asserts `result.returncode == 0`) is the canonical correct pattern that the new `tests/parity/conftest.py` buf-parity harness initially failed to follow. The companion doc captures the broader rule: every subprocess wrapper in a test harness must enumerate its tool's documented success exit codes as a `frozenset` constant and assert membership before parsing stdout. The two docs together cover the discipline at two layers: this doc validates the gate's own subprocess (mypy / ruff exit codes) using the pattern; the new doc generalizes the pattern for any external-tool wrapper.
- [[presence-ratchet-test-pattern-for-prose-substrings]] — sibling ratchet pattern: this doc pins file-set membership in a gated-paths tuple; the presence-ratchet pins prose substrings in docstrings / CHANGELOG sections / README disclaimers. Both share the "ratchet against silent regression" philosophy with deliberate-update prompts in the failure message; they differ in what the assertion contracts on (tuple equality vs substring presence) and the regression class they catch (file additions vs documentation reverts).
- [[behavior-preserving-test-move-breaks-path-coupling-2026-06-13]] — the test-directory reorganization that exercised this gate's two coupling classes (the `_LINT_PATHS` path-string list and the `_REPO_ROOT` depth anchor). It generalizes the depth caveat above into a four-class taxonomy of what a "behavior-preserving" test-file move breaks, and documents how this gate's entries were repointed and its anchor deepened to `parents[2]`.
