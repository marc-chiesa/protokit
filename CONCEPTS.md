...
Streams are one use, not the definition: every pool protokit builds from a descriptor set or a compile is isolated, and none is the process-wide default pool that generated code registers into. So anything keyed to default-pool identity breaks on all of them — including protobuf's own built-in options classes, which know only default-pool custom options and refuse any other pool's by identity even when the names match. The refusal is an error, but it reads exactly like "not set" to code that catches it, so a custom option is read by re-reading the options through the pool that declares it, never by catching the refusal.
...
### Seam
A module that owns one recurring decision for the whole package — how a message's fields are enumerated, how a custom option is resolved, how a result type is frozen — so that the next defect in that class is fixed once, at the owner, rather than at each call site that happens to report it. What makes it a seam rather than a convention is that reaching around it fails: a seam lands together with a guard test that fails when a call site bypasses the owner. Where no test can recognise a bypass, the seam is guarded by construction instead: the owner is the only thing that exposes the means of doing the job, and its guard pins that exposure while saying plainly that a rebuild from scratch is beyond it. A call site that re-derives the owner's answer and happens to agree has reached around it without the guard noticing, so the guard exercises every way the owner can decide rather than one case per call site. Where the owner's use is decidable by running each consumer, the guard can be a reflection over the consumers instead; it is then only as complete as its enumeration, so a consumer it cannot find or classify fails by name rather than passing as out of scope.
